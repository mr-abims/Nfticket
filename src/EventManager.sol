...
contract EventManager is ERC721, ERC721URIStorage, Ownable {
   ...
   event UserRegistered(
    ...
    function createEvent(
        ...
        if (_ticketFee > 0) {
            IdtoEvent[eventId] = Event(_eventName, eventId, _regStartBlock, _regEndblock, _ticketFee, true, _maxTickets, 0, _ticketURI);
        } else {
            IdtoEvent[eventId] = Event(_eventName, eventId, _regStartBlock, _regEndblock, 0, false, _maxTickets, 0, _ticketURI);
        }
        users[msg.sender].eventsCreated.push(uint(eventId));
    ...
    function registerForEvent(bytes32 eventId) public payable {
        ...
        users[msg.sender].eventsAttended.push(uint(eventId));
    ...
    function getUserEvents(address user) public view returns (uint[] memory, uint[] memory) {
        return (users[user].eventsAttended, users[user].eventsCreated);
    }
    function getUserAttendedEvents(address user) public view returns (uint[] memory) {
        return users[user].eventsAttended;
    }
    function getUserCreatedEvents(address user) public view returns (uint[] memory) {
