...
contract EventManagerTest is Test {
    ...
    function test_factoryFunctionality() public {
        ...
        eventFactory.createEvent("Event 2", "E2", block.timestamp + 200, block.timestamp + 2000, 0.01 ether, 100, "ipfs://event2");
        vm.stopPrank();
    ...
    function test_getLiveAndPastEvents() public {
        vm.startPrank(owner);
        // Create a live event
        eventFactory.createEvent("Live Event","LE", block.timestamp + 100, block.timestamp + 1000, 0, 50, "ipfs://live");
        
        // Create a past event (that will be ended manually)
        address pastEventAddress = eventFactory.createEvent("Past Event", "PE", block.timestamp + 50, block.timestamp + 200, 0, 50, "ipfs://past");
    ...
    function test_multipleEventCreation() public {
        ...
        vm.startPrank(owner);
        address event1 = eventFactory.createEvent("Event 1", "E1", block.timestamp + 100, block.timestamp + 1000, 0, 50, "ipfs://event1");
        address event2 = eventFactory.createEvent("Event 2", "E2", block.timestamp + 200, block.timestamp + 2000, 0.01 ether, 100, "ipfs://event2");
        vm.stopPrank();
    ...
    function test_eventNameUniqueness() public {
        ...
        vm.prank(owner);
        eventFactory.createEvent("Unique Event", "UE", block.timestamp + 100, block.timestamp + 1000, 0, 50, "ipfs://event1");
        
        // Try to create another event with the same name - should fail
        vm.prank(owner);
        vm.expectRevert("Event name already exists");
        eventFactory.createEvent("Unique Event", "UE2", block.timestamp + 200, block.timestamp + 2000, 0, 100, "ipfs://event2");
    ...
    function test_eventStatusTracking() public {
        ...
        vm.startPrank(owner);
        address liveEvent = eventFactory.createEvent("Live Event", "LE", block.timestamp + 100, block.timestamp + 1000, 0, 50, "ipfs://live");
        address pastEvent = eventFactory.createEvent("Past Event", "PE", block.timestamp + 50, block.timestamp + 200, 0, 50, "ipfs://past");
        vm.stopPrank();
